...
class CSRfile extends Module
{
    ...
    val csr_write_data = MuxCase(csr_read_data,Array(
        ...
        (io.csr_op === csr_s) -> (io.data_in | csr_read_data),
        (io.csr_op === csr_c) -> (io.data_in & (~csr_read_data))
    ))

    val csr_wen = !write_illgal && legalcsrnumber && !io.hasException && !io.hasStall
