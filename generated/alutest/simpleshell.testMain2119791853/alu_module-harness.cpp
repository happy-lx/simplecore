...
#endif
#include <iostream>
class alu_module_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    alu_module_api_t(Valu_module* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_res_ext_op)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_op)));
        sim_data.inputs.push_back(new VerilatorQData(&(dut->io_input2)));
        sim_data.inputs.push_back(new VerilatorQData(&(dut->io_input1)));
        sim_data.outputs.push_back(new VerilatorQData(&(dut->io_res)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["alu_module.reset"] = 0;
    }
#if VM_TRACE
...
static alu_module_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    Valu_module* top = new Valu_module;
    std::string vcdfile = "./generated/alutest/simpleshell.testMain2119791853/alu_module.vcd";
